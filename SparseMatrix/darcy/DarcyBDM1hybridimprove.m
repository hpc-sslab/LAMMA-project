function [u,eqn,info] = DarcyBDM1hybridimprove(roughu,node,elem,pde,bdFlag,option,hybrid_parameter)
%%  solve u only
...
%
% (div u^n, div v) + hybrid_parameter*(d^{-1} u^n,v) = -(f, div v) + hybrid_parameter*(d^{-1} u^0, v)
%
% added by I. Kim
...
[AD,F,u,freeEdge,isPureNeumannBC] = getbdhyBDM1improve(F,hybrid_parameter);
...
function [AD,F,u,freeDof,isPureNeumannBC] = getbdhyBDM1improve(F,hybrid_parameter)
...
%B = icdmat(double(elem2edge),elemSign*[1 -1 1]);
    ...
    if ~isempty(pde.g_N) && any(isNeumann)
        ...
        F(idpsi(isNeumann))=u(idpsi(isNeumann));
