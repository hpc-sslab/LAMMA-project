...
locEdge = [2 3; 3 1; 1 2];%not ascending order

%% Compute square of the L2 error element-wise
[lambda,w] = quadpts(4); % quadrature order is 5
nQuad = size(lambda,1);
err = zeros(NT,1);
...
for p = 1:nQuad
    ...
    for k = 1:3 % for each basis
        ...
        repmat(elem2edgeSign(:,k).*sigmah(elem2dof(:,k)),1,2);
  ...
  %figure(1); 
  %showsolutionRT(node,elem,sigmap); pause(0.1);
  %figure(2);
  %showsolutionRT(node,elem,sigmahp); pause(0.1);
