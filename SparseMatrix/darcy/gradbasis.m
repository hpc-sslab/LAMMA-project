...
% findnode(node);
% findelem(node,elem);
% midEdge = (node(elem(:,3),:)+node(elem(:,2),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),ve1(:,1),ve1(:,2));
% midEdge = (node(elem(:,3),:)+node(elem(:,1),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),ve2(:,1),ve2(:,2));
% midEdge = (node(elem(:,1),:)+node(elem(:,2),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),ve3(:,1),ve3(:,2));
% 

%inward normal with magnitude edgeLength
%Dlambda=grad lambda_i=1/(2*area)*inwardnormalvector
...
% findelem(node,elem);
% midEdge = (node(elem(:,3),:)+node(elem(:,2),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),Dlambda(:,1,1),Dlambda(:,2,1));
% midEdge = (node(elem(:,3),:)+node(elem(:,1),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),Dlambda(:,1,2),Dlambda(:,2,2));
% midEdge = (node(elem(:,1),:)+node(elem(:,2),:))/2;
% quiver(midEdge(:,1),midEdge(:,2),Dlambda(:,1,3),Dlambda(:,2,3));
% 
