function[u,eqn,info] = DarcyRT0hybridimprove(roughu,node,elem,pde,bdFlag,option,hybrid_parameter)%1 dim uI on edges
  %%  solve u only, roughu coefficients in step 1
  %  [u,eqn,info] = DarcyRT0hybridmod(roughu,node,elem,pde,bdFlag) produces an approximation of
...
%
%
% (div u^new, div v) + delta*(d^{-1} u^new,v) = -(f, div v) + delta*(d^{-1} u^old,v)
%
% added by I. Kim
...
F(1:Ndof,1) = fu + hybrid_parameter.*M*roughu;


if ~exist('bdFlag','var'), bdFlag = []; end
[AD,F,u,freeEdge,freeDof,isPureNeumannBC] = getbdhyRT0improve(F,hybrid_parameter);
...
function [AD,F,u,freeEdge,freeDof,isPureNeumannBC] = getbdhyRT0improve(F,hybrid_parameter)
...
%        % modify the rhs to include Dirichlet boundary condition 
        ...
        %u(isNeumann) = evalg_N.*edgeSign(isNeumann);
