...
#ifdef __STDC__
void newPB_COutV( PBTYP_T * TYPE, char * ROWCOL, char * ZEROIT, int M,
               int N, int * DESCA, int K, char * * YAPTR, int * DYA,
               int * YAFREE, int * YASUM )
#else
void newPB_COutV( TYPE, ROWCOL, ZEROIT, M, N, DESCA, K, YAPTR, DYA,
               YAFREE, YASUM )
/*
*  .. Scalar Arguments ..
*/
...
*/
         if( Anq > 0 )
         {
            *YAPTR  = PB_Chbwmalloc( K * Anq * TYPE->size );
            *YAFREE = 1;
...
*/
         if( Amp > 0 )
         {
            *YAPTR  = PB_Chbwmalloc( Amp * K * TYPE->size );
            *YAFREE = 1;
            if( Mupcase( ZEROIT[0] ) == CINIT )
            {
               zero = TYPE->zero;
