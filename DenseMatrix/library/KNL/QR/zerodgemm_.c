...
// Packing A, c-major -> r-major
void acr(int           row, // # of rows
        int           col, // # of cols
        const double *mt, //  from *mt
        int           inc, // distance
        double       *bk ) // to *bk
{
    int q = col / MR; // quotient
    int r = col % MR; // remainder
    int i, j;
    omp_set_nested(1); // just in case
    // main part
    ...
    for(i = 0; i < q; ++i)
    {
        for(j = 0; j < row; ++j)
        {
            // copy MR elements from &mt[j+i*MR*inc] with interval inc
            // loop order is i-loop -> j-loop to access bk contiguously
            bk[j*MR+i*row*MR:MR]=mt[j+i*MR*inc:MR:inc];
        }
    }
    // move pointers
    bk += q*row*MR;
	...
	//
	{
		...
		#pragma omp parallel num_threads(NT1) private(_B, nc, j)
		{
			...
			for(j = 0; j < nq; ++j)
			{
				...
				bcr(k,nc,&B[ki*KB+j*NB*lb],lb,_B);
				#pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
				{
					...
					for(ir = 0; ir <mq ; ++ir)
					{
						...
						for(p = 0; p < pq; ++p)
						{
							...
							if(pc == NR && mc == MR)
							{
								kernel1(k,&A[ir*MR*k],&_B[p*NR*k],&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],lc);
							}
							else
							{
								kernel1(k,&A[ir*MR*k],&_B[p*NR*k],_C,MR);
								dxpy(mc,pc,&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],_C,lc);
							}
						}
					}
				}
        ...
        const int              lc)
{
    ...
    //
    {
        ...
        #pragma omp parallel num_threads(NT1) private(_B, nc, j)
        {
            ...
            {
                nc = (j != nq-1 || nd == 0) ? NB : nd;
                brr(k,nc,&B[ki*KB*lb+j*NB],lb,_B);
                #pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
                {
                    #pragma omp for //schedule(dynamic)
                    // ir-loop
...
if(nota){
  if(notb){  // N x N
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		for(l = 0; l < kq; ++l)
		{
			...
			acc(mc,kc,&A[ii*MB+l*KB*la],la,_A);
				//j-ir loop
			jir(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
		}
	}
  }
  else{  // N x T
	for(ii = 0; ii < mq; ++ii)
	{
		...
		for(l = 0; l < kq; ++l)
		{
			...
			acc(mc,kc,&A[ii*MB+l*KB*la],la,_A);
				//j-ir loop
			jirT(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
		}
	}
  }
}
  ...
  if(notb){  // T x N
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		{
			kc = (l != kq-1 || kd == 0) ? KB : kd;
			arc(mc,kc,&A[ii*MB*la+l*KB],la,_A); // packing A row -> column 

//			acr(kc,mc,&A[ii*MB*la+l*KB],la,_A); // packing A column -> row
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		{
			kc = (l != kq-1 || kd == 0) ? KB : kd;
			arc(mc,kc,&A[ii*MB*la+l*KB],la,_A);
				//j-ir loop
			jirT(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
		}
	}	
