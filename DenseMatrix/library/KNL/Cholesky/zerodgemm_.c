...
void microkernel0(int k, const double * A, const double * B, double * C, int ncol)
{
  int i;
...
void microkernel1(int k, const double * A, const double * B, double * C, int ncol)
{
  int i;
  register __m512d _A0;
  register __m512d _C0_0, _C1_0, _C2_0, _C3_0, _C4_0, _C5_0, _C6_0, _C7_0, _C8_0, _C9_0, _C10_0, _C11_0, _C12_0, _C13_0, _C14_0, _C15_0, _C16_0, _C17_0, _C18_0, _C19_0, _C20_0, _C21_0, _C22_0, _C23_0, _C24_0, _C25_0, _C26_0, _C27_0, _C28_0, _C29_0, _C30_0;
	...
	//
	{
		...
		#pragma omp parallel num_threads(NT1) private(_B, nc, j)
		{
			...
			for(j = 0; j < nq; ++j)
			{
				...
				pbcr(k,nc,&B[ki*KB+j*NB*lb],lb,_B);
				#pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
				{
					...
					for(ir = 0; ir <mq ; ++ir)
					{
						...
						for(p = 0; p < pq; ++p)
						{
							...
							if(pc == NR && mc == MR)
							{
								microkernel1(k,&A[ir*MR*k],&_B[p*NR*k],&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],lc);
							}
							else
							{
								microkernel1(k,&A[ir*MR*k],&_B[p*NR*k],_C,MR);
								microdxpy(mc,pc,&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],_C,lc);
							}
						}
					}
				}
        ...
        const int              lc)
{
    ...
    //
    {
        ...
        #pragma omp parallel num_threads(NT1) private(_B, nc, j)
        {
            ...
            {
                nc = (j != nq-1 || nd == 0) ? NB : nd;
                pbrr(k,nc,&B[ki*KB*lb+j*NB],lb,_B);
                #pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
                {
                    #pragma omp for //schedule(dynamic)
                    // ir-loop
...
if(nota){
  if(notb){  // N x N
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		for(l = 0; l < kq; ++l)
		{
			...
			pacc(mc,kc,&A[ii*MB+l*KB*la],la,_A);
				//j-ir loop
				jir_loop(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
			}
		}
	}
  else{  // N x T
	for(ii = 0; ii < mq; ++ii)
	{
		...
		for(l = 0; l < kq; ++l)
		{
			...
			pacc(mc,kc,&A[ii*MB+l*KB*la],la,_A);
				//j-ir loop
				jir_loopT(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
			}
		}
	}
}
  ...
  if(notb){  // T x N
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		{
			kc = (l != kq-1 || kd == 0) ? KB : kd;
				parc(mc,kc,&A[ii*MB*la+l*KB],la,_A);
				//j-ir loop
				jir_loop(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
			}
		}
