...
double validTest(kernel_t *kernel, int* count){
	...
	sprintf(openCommand, "speedRecord/%s.out", kernelName);
	(*count)++;
...
double microKernelFlopsTest(kernel_t *kernel, int* count, int isNeedRetest){
	FILE* fp;
	char kernelName[256];
	char openCommand[512];
	char makeCommand[512];
	char executePath[256] = "kernel";
	char recordPath[256] = "speedRecord";
	char action[256] = "flopsMicroKernel";
	char action2[256] = "executeMicroKernel";
	double flops = -100.0;
	double maxflops = -100.0;
	char getString[256];
	int flag=0;
	...
	sprintf(kernelName, "%d.%dx%dkernel%dx%dx%d_%dx%dx%d_%d+%d", 
			kernel->nPack, kernel->ni, kernel->njr, kernel->mu, kernel->nu, kernel->ku, 
			kernel->mb, kernel->nb, kernel->kb, kernel->prefetchA1, kernel->prefetchB1);
	sprintf(openCommand, "%s/%s.flops", executePath, kernelName);
	fp = fopen(openCommand, "r");
	if(!fp){
		sprintf(makeCommand, "make %s KERNEL=%s M=%d N=%d K=%d NT=%d NT1=%d NT2=%d", action, kernelName,
				kernel->mu, kernel->nu, kernel->kb, kernel->nPack, kernel->ni, kernel->njr);
		printf("make : %s\n", makeCommand);
		system(makeCommand);
	}else{
		fclose(fp);
	...
	// check is there result file already
	sprintf(openCommand, "%s/%s.res", recordPath, kernelName);
	fp = fopen(openCommand, "r");
	if(!fp){
		isNeedRetest = 1;
	}else{
	...
	// if there is no result || need to retest -> execute "executeKernel"
	if(isNeedRetest){
		sprintf(makeCommand, "make %s KERNEL=%s NT=%d NT1=%d NT2=%d" , action2, kernelName,
	...
	fp = fopen(openCommand, "r");
	while(fgets(getString, 256, fp)){
		sscanf(getString, "average Gflops : %lf", &flops);
		if(flops > maxflops){
			maxflops=flops;
		}
	}
	printf("%c[31m", 27);
	printf("total Gflops = %lf", maxflops);
	printf("%c[0m\n", 27);
	fclose(fp);
...
double blockFlopsTest(kernel_t *kernel, int* count, int isNeedRetest){
	FILE* fp;
	char kernelName[256];
	char openCommand[512];
	char makeCommand[512];
	char executePath[256] = "kernel";
	char recordPath[256] = "speedRecord";
	char action[256] = "flopsBlockKernel";
	char action2[256] = "executeBlockKernel";
	double flops = -100.0;
	double maxflops = -100.0;
	char getString[256];
	int flag=0;
	...
	if(!fp){
		sprintf(makeCommand, "make %s KERNEL=%s M=%d N=%d K=%d NT=%d NT1=%d NT2=%d", action, kernelName,
				kernel->mb, kernel->nb, kernel->kb, kernel->nPack, kernel->ni, kernel->njr);
		printf("make : %s\n", makeCommand);
		system(makeCommand);
	}else{
		fclose(fp);
