...
vector_t vector = {8, "__m512d", "_mm512_setzero_pd", "_mm512_loadu_pd", "_mm512_storeu_pd", 
...
void printCLoad(FILE* fp, int id){
	int a,b,offset;
	...
	if(id == 0){
		...
		for(a=0; a<genKernel.mu; a++){
			for(b=0; b<genKernel.nu/vector.vector_stride; b++){
				fprintf(fp, "    _C%d_%d = %s();\n", b, a, vector.vector_setzero);
			}
		}
	}
}
 ...
 * print function
 */
void printFunction(FILE* fp, int id){
	int a, b;
	//print function name
	fprintf(fp, "void micro_kernel%d(int k, const double * A, const double * B, double * C, int ncol)\n", id);
	fprintf(fp, "{\n");
...
void generateMicroKernelCode(FILE* fp, kernel_t* kernel){
...
void attachOuterLoop(const char* kernelName){
	char command[1024];
	sprintf(command, "cat outerLoop.c >> %s", kernelName);
	assert(!system(command));
}


void attachBlockOuterLoop(const char* kernelName){
	char command[1024];
	sprintf(command, "cat block_outerLoop.c >> %s", kernelName);
	assert(!system(command));
}
...
void generateMicroKernel(kernel_t* kernel){
	FILE* fp;
	char kernelName[512];
	char path[256] = "kernel";

	/*
	sprintf(kernelName, "%s/microKernel.c", path);
	printf("microkernel Name = %s mu=%d, nu=%d, ku=%d, prefetchA1=%d, prefetchB1=%d\n", 
			kernelName, kernel->mu, kernel->nu, kernel->ku, kernel->prefetchA1, kernel->prefetchB1);
	*/
	sprintf(kernelName, "%s/%d.%dx%dkernel%dx%dx%d_%dx%dx%d_%d+%d.c", path, 
			kernel->nPack, kernel->ni, kernel->njr, kernel->mu, kernel->nu, kernel->ku,
			kernel->mb, kernel->nb, kernel->kb, kernel->prefetchA1, kernel->prefetchB1);
	...
	//fp = fopen(kernelName, "a+");
	fp = fopen(kernelName, "w");
	assert(fp);
	generateKernelCode(fp, kernel);
 	fclose(fp);
	printf("it is generated.\n");
}


void generateBlockKernel(kernel_t* kernel){
	FILE* fp;
	char kernelName[512];
	char path[256] = "kernel";
	...
	*/
	...
	generateKernelCode(fp, kernel);
 	fclose(fp);
	printf("it is generated.\n");
	attachBlockOuterLoop(kernelName);
	printf("outerLoop is attached.\n");
}
...
int microKernelConstraint(kernel_t* kernel){
	if(kernel->mu <= 0) return -1;
	if(kernel->nu <= 0 || kernel->nu % vector.vector_stride != 0) return -1;
...
int blockKernelConstraint(kernel_t* kernel){
	if(kernel->mu <= 0) return -1;
	if(kernel->nu <= 0 || kernel->nu % vector.vector_stride != 0) return -1;
...
int prepareMicroKernel(kernel_t* kernel){
	  if(microKernelConstraint(kernel) == -1) return -1;
		generateMicroKernel(kernel);
		return 0;
}

int prepareBlockKernel(kernel_t* kernel){
	  if(blockKernelConstraint(kernel) == -1) return -1;
		generateBlockKernel(kernel);
		return 0;
}
