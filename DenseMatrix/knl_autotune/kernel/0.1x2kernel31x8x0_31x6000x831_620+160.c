             ...
             const int              lc)
{
  ...
  int kt = MB/MR;
  //
//  double _A[MB*KB] __attribute__((aligned(32)));
...
//      packarc(mc,k,&A[ki*KB+i*MB*la],la,_A);
	#pragma omp parallel num_threads(NT2) private(j,nc,pq,p,pd,pc,_C) shared(A,mc)
       {
       #pragma omp for //schedule(dynamic)
        for(j = 0; j < nq; ++j)
        {
          ...
          for(p = 0; p < pq; ++p)
          {
            ...
            if(pc == MR && nc == NR)
           {
			  micro_kernel0(k,&A[p*(MR)*k],&B[j*NR*k],&C[i*MB*lc+p*MR*lc+ni*NB+j*NR],lc);
            }
//            else
//            {
