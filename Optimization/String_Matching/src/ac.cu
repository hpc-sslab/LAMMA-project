...
int GetValueOfPattern(ACSM_STRUCT2 *acsm, unsigned char *pattern, int len ){
	ACSM_PATTERN2 *pat;
	int i, pos, flag;
	pat = (ACSM_PATTERN2 *)AC_MALLOC(sizeof (ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
	MEMASSERT (pat, "GetValueOfPattern");

	pat = acsm->acsmPatterns;
	pos = 0;
	flag = 1;
...
void Array_Patterns( ACSM_STRUCT2 * acsm, acstate_t *arr_dfa, int *arr_ptn)
{
	ACSM_PATTERN2 * mlist;
	int state, j, k ; 
	k = 0;
	for(state = 0; state<acsm->acsmNumStates; state++) //duyet qua cac trang thai
	{
		...
		j = arr_dfa[state * (acsm->acsmAlphabetSize+2) + 1];
		if(j==1){
			for (mlist = acsm->acsmMatchList[state];  mlist;  mlist = mlist->next)
			{
				...
				arr_ptn[k++] = state; 
				arr_ptn[k++] = GetValueOfPattern(acsm, mlist->patrn, mlist->n);
			}
		}
	}
}
int GetNumOfPatterns(ACSM_STRUCT2 *acsm)
{
	ACSM_PATTERN2 * mlist;
	int state, n; 
	n = 0;
...
/*void Print_DFA_Link(ACSM_STRUCT2 *acsm)
{
	Print_DFA(acsm);
}*/
...
int acsmPrintSummaryInfo2(void)
{
	...
	printf("+- [ Aho-Corasick Summary ] -------------------------------------\n");
	printf("| Storage Format    : %s \n",sf[ p->acsmFormat ]);
	printf("| Finite Automaton  : %s\n", fsa[p->acsmFSA]);
	printf("| Alphabet Size     : %d Chars\n",p->acsmAlphabetSize);

	if (summary.acsm.compress_states)
		printf("| Sizeof State      : Variable (1,2,4 bytes)\n");
	else
		printf("| Sizeof State      : %d bytes\n",(int)(sizeof(acstate_t)));
	...
	if (summary.acsm.compress_states)
	{
		printf("|     1 byte states : %u\n", summary.num_1byte_instances);
		printf("|     2 byte states : %u\n", summary.num_2byte_instances);
		printf("|     4 byte states : %u\n", summary.num_4byte_instances);
	}
...
*/ 
/*int
MatchFound (void* id, void *x ,int index, void *data, void *y) 
{
	//fprintf (stdout, "%s\n", (char *) id);
	return 0;
}
...
void Print_Matrix_DFA(ACSM_STRUCT2 *acsm){
	Print_DFA(acsm);
}
