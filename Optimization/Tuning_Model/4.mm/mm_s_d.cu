...
//kernel function
template<typename T>
__global__ void MatrixMulKernel(T *P, T *M, T *N, const unsigned int width)
{
	unsigned int i;
	unsigned int r, c;
...
template<typename T>
int MatrixMul(T *P, const T *M, const T *N, unsigned int n)
{
	int i, j, k ;
	T sum, a, b ;
	for(i = 0 ;i < n; i++)
...
int main(int argc, char * argv[])
{
	...
	if(sp==SINGLE_PRECISION)
	{
		MatrixMulKernel<float><<<grid, block>>>(SPd, SMd, SNd, width);
	}
	else
	{
		MatrixMulKernel<double><<<grid, block>>>(DPd, DMd, DNd, width);
	}
	...
	if(sp==SINGLE_PRECISION)
	{
		...
		*/
		MatrixMul<float>(SCP, SM, SN, width);
		//printf("CP >>>>>>>>>>>>>>>>>>>>>>>>>>\n");
	...
	else
	{
		...
		*/
		MatrixMul<double>(DCP, DM, DN, width);
		//printf("CP >>>>>>>>>>>>>>>>>>>>>>>>>>\n");
